...
const PropertiesList: React.FC<PropertiesListProps> = ({ expenses }) => {
  ...
  const addRoom = (propertyId: string) => {
    setProperties(properties.map(p => {
      if (p.id === propertyId) {
        return {
          ...p,
          rooms: [...p.rooms, { id: Date.now().toString(), number: 'New Room' }]
        };
      }
      return p;
    }));
  ...
  const savePropertyInfo = (id: string) => {
    setProperties(properties.map(p => p.id === id ? { ...p, name: tempName, address: tempAddress } : p));
    setEditingPropertyId(null);
  };

  const saveRoomNumber = (propertyId: string, roomId: string) => {
    setProperties(properties.map(p => {
      if (p.id === propertyId) {
        return {
          ...p,
          rooms: p.rooms.map(r => r.id === roomId ? { ...r, number: tempName } : r)
        };
      }
      return p;
    }));
                  ...
                  <div key={room.id} className="group relative flex items-center justify-between px-3.5 py-3 rounded-2xl bg-slate-50/50 dark:bg-background-dark border border-slate-200 dark:border-white/5">
                    {editingRoomId === room.id ? (
                      <input 
                        ...
                        value={tempName}
                        onChange={(e) => setTempName(e.target.value)}
                        onBlur={() => saveRoomNumber(property.id, room.id)}
                        onKeyDown={(e) => e.key === 'Enter' && saveRoomNumber(property.id, room.id)}
                      />
                    ) : (
                      <>
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button 
                            onClick={() => deleteRoom(property.id, room.id)}
