...
const TenantsList: React.FC<TenantsListProps> = ({ tenants, onSelectTenant, onAddTenant }) => {
  ...
  const [newTenantData, setNewTenantData] = useState({
    name: '',
    property: MOCK_PROPERTIES[0]?.name || '',
    unit: '',
    rent: '',
    deposit: '',
  ...
  const availableUnits = useMemo(() => {
    const prop = MOCK_PROPERTIES.find(p => p.name === newTenantData.property);
    return prop ? prop.rooms : [];
  }, [newTenantData.property]);
